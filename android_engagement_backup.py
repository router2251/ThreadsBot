...
class AIAnalyzer:
    ...
    def analyze_user_profile(self, username: str, bio: str, comment_text: str) -> Dict:
        """Use AI to analyze user profile for demographics and characteristics."""
        try:
            prompt = f"""
            Analyze this social media user profile and provide detailed insights:
            
            Username: {username}
            Bio: {bio}
            Recent Comment: {comment_text}
            
            Please analyze and return a JSON response with the following information:
            - likely_gender (male/female/unknown)
            - likely_age_range (teens/20s/30s/40s/50s+/unknown)
...
class AndroidEngagement:
        ...
        try:
            ...
            for scroll_attempt in range(self.max_posts_to_scan):
                try:
                    ...
                    for i, heart_icon in enumerate(heart_icons):
                        try:
                            ...
                            if len(nearby_numbers) >= 1:  # Changed from 2 to 1
                                ...
                                # Fallback: check if any numbers on screen meet viral criteria
                                for element, num, text, field_type in elements_with_numbers:
                                    if num >= self.min_likes_for_viral:
                                        # Verify this is actually a post by checking heart icon content
                                        try:
                                            heart_content_desc = heart_icon.attrib.get('content-desc', '')
                                            if 'like' in heart_content_desc.lower() or 'unlike' in heart_content_desc.lower():
                                                logger.info(f"  Found viral-level engagement: {num} from {field_type}: '{text}'")
                                                viral_post = {
                                                    'heart_icon': heart_icon,
                                                    'like_count': num,
                                                    'comment_count': 1,  # Assume at least 1 comment
                                                    'position': heart_center
                                                }
                                                viral_posts.append(viral_post)
                                                logger.info(f"  Added viral post to list via fallback. Total viral posts: {len(viral_posts)}")
                                                break
                                            else:
                                                logger.info(f"  Heart icon does not have like/unlike content: '{heart_content_desc}' - skipping fallback")
                                        except Exception as e:
                                            logger.error(f"  Error checking heart icon content in fallback: {e}")
                                            continue
                        except Exception as e:
                    ...
                    if viral_posts:
                        logger.info(f"Found {len(viral_posts)} viral posts, stopping scan and processing...")
                        break
                    else:
                        logger.info("No viral posts found in this scan attempt")
                        
                    logger.info("No viral posts found, scrolling down to load more content...")
                    device.swipe(0.5, 0.8, 0.5, 0.2)
                    time.sleep(3)
                    
                except Exception as e:
                    logger.error(f"Error in feed scanning: {str(e)}")
                    break
                    
            logger.info(f"Found {len(viral_posts)} viral posts to examine")
            return viral_posts
    ...
    def extract_number_from_text(self, text: str) -> int:
        ...
        try:
            ...
            # Remove common suffixes like "likes", "comments", "K", "M"
            import re
            # Find numbers (including K for thousands, M for millions)
            numbers = re.findall(r'(\d+(?:\.\d+)?[KMB]?)', text.replace(',', ''))
            
            logger.debug(f"Found numbers in '{text}': {numbers}")
            
            if numbers:
                num_str = numbers[0].upper()
                if num_str.endswith('K'):
                    result = int(float(num_str[:-1]) * 1000)
                elif num_str.endswith('M'):
                    result = int(float(num_str[:-1]) * 1000000)
                elif num_str.endswith('B'):
                    result = int(float(num_str[:-1]) * 1000000000)
                else:
        ...
        try:
            ...
            try:
                ...
                if heart_bounds:
                    ...
                    logger.error("No bounds found for heart element")
                    return
